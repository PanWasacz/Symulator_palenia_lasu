...
WEATHER_PRESETS = {
    'very_wet': {
        ...
        'multiplier': 0.3,
        'description': 'Niedawny deszcz, las mokry',
        'color': (100, 150, 255)
    },
    'wet': {
        'name': 'Wilgotny',
        'multiplier': 0.6,
        'description': 'Poranna rosa, srednia wilgotnosc',
        'color': (150, 200, 255)
    },
    'normal': {
        'name': 'Normalny',
        'multiplier': 1.0,
        'description': 'Standardowe warunki',
        'color': (200, 200, 200)
    },
    'dry': {
        'name': 'Suchy',
        'multiplier': 1.5,
        'description': 'Upalna pogoda, niska wilgotnosc',
        'color': (255, 200, 100)
    },
    'very_dry': {
        'name': 'Bardzo suchy',
        'multiplier': 2.2,
        'description': 'Dluga susza, wyjatkowo sucho',
        'color': (255, 150, 50)
    },
    'extreme': {
        'name': 'EKSTREMALNY',
        'multiplier': 3.5,
        'description': 'Czerwony alarm - super sucho',
        'color': (255, 50, 0)
    }
}
...
pygame.init()
pygame.display.set_caption("Symulacja Pozaru Lasu v6.1 - Kontrola Szybkosci + Pauza")
...
class ForestFireSimulation:
    def __init__(self, width, height, cell_size):
        ...
        # PRĘDKOŚĆ SYMULACJI
        self.simulation_speed = 1.0  # 0.1 = bardzo wolno, 1.0 = normalnie, 5.0 = szybko
        self.update_counter = 0

        # SYSTEM SZYBKOŚCI SPALANIA
    ...
    def update(self):
        ...
        # Kontrola prędkości - aktualizuj tylko co N-tą klatkę
        self.update_counter += self.simulation_speed

        if self.update_counter < 1.0:
            return  # Pomiń tę klatkę

        # Wykonaj tyle kroków ile się uzbierało
                ...
                if state == FIRE:
                    ...
                    if new_fire[y][x] <= 0:
                        ...
                        for nx, ny, w_factor in self.get_neighbors(x, y):
                            ...
                            if n_state in [WATER, ROCK, FIREBREAK]:
                                continue
    ...
    def draw(self, surface):
        ...
        if self.cutting_mode:
            text_surf = font.render("TRYB WYCINANIA", True, (255, 200, 0))
        ...
        # Wskaźnik prędkości (jeśli nie standardowa)
        if self.simulation_speed != 1.0:
            if self.simulation_speed < 1.0:
                speed_text = f"WOLNIEJ {self.simulation_speed:.1f}x"
                speed_color = (100, 150, 255)
            else:
                speed_text = f"SZYBCIEJ {self.simulation_speed:.1f}x"
                speed_color = (255, 100, 100)

            speed_surf = small_font.render(speed_text, True, speed_color)
            speed_rect = speed_surf.get_rect(topleft=(10, 40))
    ...
    def draw_ui(self, surface):
        ...
        weather_box = pygame.Rect(ui_x - 5, y - 5, self.ui_width - 30, 100)
        pygame.draw.rect(surface, (30, 30, 30), weather_box)
        pygame.draw.rect(surface, weather_info['color'], weather_box, 3)

        weather_text = font.render(weather_info['name'], True, weather_info['color'])
        surface.blit(weather_text, (ui_x + 5, y))
        y += 28

        mult_text = small_font.render(f"Mnoznik spalania: {self.burn_rate_multiplier}x",
                                      True, (255, 255, 255))
        surface.blit(mult_text, (ui_x + 5, y))
        y += 20

        desc_lines = self.wrap_text(weather_info['description'], 40)
        for line in desc_lines:
            desc_text = tiny_font.render(line, True, (200, 200, 200))
            surface.blit(desc_text, (ui_x + 5, y))
            y += 16

        y += 10

        params_title = small_font.render("Parametry:", True, (200, 200, 200))
        surface.blit(params_title, (ui_x + 5, y))
        y += 18

        params = [
            f"Rozprzestrzenianie: {self.p_spread:.3f}",
            f"Wypalanie: {self.fire_decay:.3f}",
            f"Wzrost lasu: {self.p_grow:.5f}",
        ]

        for param in params:
            p_text = tiny_font.render(param, True, (180, 180, 180))
            surface.blit(p_text, (ui_x + 10, y))
            y += 15
        ...
        for name, state_id in legend_items:
            ...
            pygame.draw.rect(surface, COLORS[state_id], (ui_x, y, 15, 15))
            pygame.draw.rect(surface, (100, 100, 100), (ui_x, y, 15, 15), 1)

            text_str = f"{name}: {pct:.1f}%"
            text = tiny_font.render(text_str, True, (200, 200, 200))
            surface.blit(text, (ui_x + 20, y + 2))
            y += 18
        ...
        dims = [
            ...
            f"Status: {'PAUZA' if self.paused else 'DZIALA'}",
        ]

        for line in dims:
            t = tiny_font.render(line, True, (180, 180, 180))
        ...
        controls_title = small_font.render("STEROWANIE:", True, (255, 255, 0))
        surface.blit(controls_title, (ui_x, y))
        y += 20

        controls = [
            "WARUNKI POGODOWE:",
            "1: Bardzo wilgotny (0.3x)",
            "2: Wilgotny (0.6x)",
            "3: Normalny (1.0x)",
            "4: Suchy (1.5x)",
            "5: Bardzo suchy (2.2x)",
        ...
        for c in controls:
            if c.startswith("WARUNKI") or c.startswith("PODSTAWOWE") or c.startswith("STEROWANIE"):
                t = tiny_font.render(c, True, (255, 255, 100))
            else:
                t = tiny_font.render(c, True, (180, 180, 180))
            surface.blit(t, (ui_x, y))
            y += 14
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif event.key == pygame.K_0:  # Reset prędkości
